...
namespace DoAn_Ver2.Areas.Admin.Controllers
{
    public class PaymentController : BaseController
    {
        ...
        public ActionResult Index(string searchString, int? page)
        {
            ...
            query = query.OrderBy(x => x.ID); // Sắp xếp theo ID
        ...
        // 2. TẠO MỚI (POST) - Có xử lý ảnh
        [HttpPost]
        [ValidateAntiForgeryToken]
        [ValidateInput(false)] // Cho phép nhập HTML ở mô tả
        public ActionResult Create(PhuongThucThanhToan model, HttpPostedFileBase uploadBtn)
        {
            if (ModelState.IsValid)
            {
                ...
                model.MaCode = model.MaCode.ToUpper().Trim(); // Chuẩn hóa mã code
        ...
        public ActionResult Edit(PhuongThucThanhToan model, HttpPostedFileBase uploadBtn)
        {
            if (ModelState.IsValid)
            {
                ...
                // Mã Code thường hạn chế sửa nếu đã phát sinh đơn hàng, nhưng ở đây cho sửa nếu cần thiết
                ...
                if (uploadBtn != null && uploadBtn.ContentLength > 0)
                {
                    ...
                    // [FIX LỖI] Tạo đường dẫn thư mục vật lý
        ...
        public JsonResult Delete(int id)
        {
            try
            {
                ...
                // Kiểm tra ràng buộc: Nếu đã có đơn hàng sử dụng phương thức này thì KHÔNG ĐƯỢC XÓA
